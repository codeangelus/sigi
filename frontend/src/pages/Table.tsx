...
export default function TabelaPecas() {
  ...
  async function fetchData() {
    ...
    try {
      const qs = buildQuery();
      const res = await fetch(`http://localhost:3000/pecas?${qs}`);
  ...
  useEffect(() => {
    fetchData();
  }, [mes, maquina, inicio, fim, page, pageSize]);

  function limpar() {
    setMes(""); setMaquina(""); setInicio(""); setFim(""); setPage(1);
  }
      ...
      <div className="overflow-auto rounded-xl ring-1 ring-black/10">
        <table className="min-w-full border-separate border-spacing-0">
          ...
          <thead className="sticky top-0 bg-white shadow-sm"> 
            <tr>
              {["Data","DV","Cliente","Maquina","Código da peça","Data de entrega","Status atraso","Quantidade","Imagem"].map((h) => (
